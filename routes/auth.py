...
def login():
    try:
        ...
        if user_password == decrypted_password or user_password == encrypted_password:
            token = encoder(User(result[0], result[1], result[2], encrypted_password, result[4], result[5], format_to_iso(result[6]), format_to_iso(result[7]), result[8], result[9]).__dict__)
